...
void TextRect::load_texture(const char *path_to_file, float w, float h) {
...
void TextRect::change_font(const std::string path_to_file) {
	this->path_to_file = path_to_file.c_str();
	load_texture();
}

void TextRect::change_text_color(const SDL_Color new_color) {
	this->text_color = new_color;
	load_texture();
}

void TextRect::change_text_size(const float new_text_size) {
	text_size = new_text_size;
	load_texture();
}

void TextRect::change_text(const std::string new_text) {
	text = new_text;
	load_texture();
}

std::string TextRect::get_text() const {
	return text;
}